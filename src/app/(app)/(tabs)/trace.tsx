...
export default function Trace() {
  ...
  const list = useMemo(() => {
    ...
    if (!q) return orders;
    return orders.filter((o) => [o.order_number, o.client?.business_name].some((f) => (f ?? '').toLowerCase().includes(q)));
  }, [orders, query]);
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, height: 46, paddingHorizontal: 14, borderRadius: 13, backgroundColor: c.surface, borderWidth: 1, borderColor: c.line, marginBottom: 16 }}>
          ...
          <TextInput
            ...
            placeholder="Buscar por N.º de orden o cliente…"
        ...
        {loading ? (
          ...
          <View style={{ gap: 12 }}>
            {list.map((o) => (
              <FolderCard key={o.id} {...orderToCard(o)} onPress={() => openOrder(o)} />
            ))}
          </View>
        )}
      </ScrollView>
...
function openOrder(o: WorkOrder) {
  router.push({ pathname: '/trace/orden/[id]', params: { id: o.id } });
}
