...
export default function Assets() {
  ...
  const list = useMemo(() => {
    ...
    return assets.filter((a) =>
      [a.name, a.tag, a.brand, a.model, a.serial].some((f) => (f ?? '').toLowerCase().includes(q)),
    );
  }, [assets, query]);
    ...
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: c.bg }}>
      ...
      >
        ...
        {loading ? (
          ...
          <View style={{ gap: 12 }}>
            {list.map((a) => (
              <FolderCard key={a.id} {...assetToCard(a)} onPress={() => openAsset(a)} />
            ))}
          </View>
        )}
      </ScrollView>
...
function openAsset(a: Asset) {
  router.push({ pathname: '/asset/[id]', params: { id: a.id } });
}
