...
import type { StockLevel, StockMovement, Warehouse } from './types';
...
/** El "Kardex": niveles del producto por depósito + sus últimos movimientos. */
export async function getProductStock(sb: SupabaseClient, productId: string): Promise<ProductStock | null> {
  const [{ data: product, error: prodErr }, { data: levels, error: lvlErr }, { data: moves, error: movErr }] = await Promise.all([
